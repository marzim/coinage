...
def loans():
    ...
    try:
        sort = request.args.get('name')
        query_loans = db.session.query(Loan, Customer).join(Customer).filter(Loan.customer_id == Customer.id).filter(Loan.is_dormant == 0).order_by(Customer.name + '.' + sort +'()')
